...
function commit (repo, config, versions) {
  versions.forEach(v => {
    config.node_js = config.node_js.map(v => v.toString());
    if (config.node_js.indexOf(v.toString()) === -1) {
      config.node_js.push(v.toString());
    }
  });
